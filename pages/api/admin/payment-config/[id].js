  ...
  try {
    ...
    if (config.provider === 'stripe') {
      if (stripePublishableKey !== undefined && !stripePublishableKey) {
        return res.status(400).json({ error: 'Stripe publishable key cannot be empty' });
      }
      if (stripeSecretKey !== undefined && !stripeSecretKey) {
        return res.status(400).json({ error: 'Stripe secret key cannot be empty' });
      }
    }
    ...
    if (config.provider === 'paypal') {
      ...
      if (paypalWebhookId !== undefined) updateData.paypalWebhookId = paypalWebhookId;
    }
    ...
    return res.status(500).json({ error: 'Failed to update payment configuration' });
  }
}
...
async function deletePaymentConfig(req, res, id) {
  try {
    ...
    return res.status(500).json({ error: 'Failed to delete payment configuration' });
  }
}