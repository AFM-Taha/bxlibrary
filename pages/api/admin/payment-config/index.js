...
async function getPaymentConfigs(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({ error: 'Failed to fetch payment configurations' })
  }
}
...
async function createPaymentConfig(req, res, user) {
  try {
    ...
    if (!provider || !['rupantor'].includes(provider)) {
      return res
        .status(400)
        .json({ error: 'Valid provider is required (rupantor)' })
    }
    ...
    if (existingConfig) {
      return res
        .status(400)
        .json({ error: `Configuration for ${provider} already exists` })
    }

    // Provider-specific validation
    if (provider === 'rupantor') {
      if (!req.body.rupantorApiKey) {
        return res
          .status(400)
          .json({ error: 'RupantorPay API Key is required' })
      }
    }
    ...
    return res
      .status(500)
      .json({ error: 'Failed to create payment configuration' })
  }
}
