...
export default function ThemeToggle({ className = '', size = 'md' }) {
  ...
  return (
    <button
      onClick={toggleTheme}
      className={`${buttonClasses[size]} rounded-lg transition-colors duration-200 ${
        isDark
          ? 'bg-secondary-700 hover:bg-secondary-600 text-warning-400'
    ...
    >
      {isDark ? (
        // Modern Sun icon for light mode
        <svg
        ...
        // Modern Moon icon for dark mode
        <svg
          className={sizeClasses[size]}
          fill='currentColor'
          viewBox='0 0 24 24'
